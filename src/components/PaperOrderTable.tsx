import { mockPaperOrders } from "@/lib/mock-data";
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="px-5 py-4 border-b border-border flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">{mockPaperOrders.length} abertas</span>
      </div>
          ...
          <tbody className="divide-y divide-border">
            {mockPaperOrders.map((o) => (
              <tr key={o.id} className="hover:bg-surface/60">
                ...
                <td className="px-4 py-3">
                  <button className="size-7 grid place-items-center rounded-md border border-border hover:bg-bear/15 hover:border-bear/40 hover:text-bear">
